...
def int_one_model(base_path,model, rho0, rs, phalo, qhalo, alpha_halo, beta_halo, gamma, dtfile):
    ...
    s_file_affix = 'rho0%4.2f_rs%4.1f_p%5.3f_q%5.3f_a%5.3f_b%5.3f_gamma%2.1f'%(rho0, rs, phalo, qhalo, alpha_halo, beta_halo, gamma)
...
#############################################
   # Grid parameter
    ...
    df = open(ppfile,'w')
    df.write(("%6.3f" % rho0) + '  '+("%4.1f" % rs) + '  '+ ("%5.3f" % phalo) + '  '+ ("%5.3f" % qhalo)+  '  '+ ("%5.1f" % alpha_halo) +  '  '+ ("%5.1f" % beta_halo)  +  '  '+ ("%5.2f" % gamma) + '  ' + ("%8.5e" % ll_vr) + '  ' + ("%8.5e" % ll_vp) + '  ' + ("%8.5e" % ll_vt) + '  ' + ("%8.5e" % ll_vr_w) + '  ' + ("%8.5e" % ll_vp_w) + '  ' + ("%8.5e" % ll_vt_w) + '  ' + ("%8.5e" % chi2_SB) )
