...
def generate_synthetic_density(
    ...
    try:
        np.savez_compressed(
            temporary,
            artifact_schema_version=np.asarray(SYNTHETIC_DENSITY_SCHEMA_VERSION, dtype=np.int64),
    ...
    return SyntheticDensityResult(
        ...
        maximum_quadrature_relative_difference=float(np.max(relative_difference)),
        median_quadrature_relative_difference=float(np.median(relative_difference)),
    )
