...
1. 以观测到的 6D 恒星为轨道种子；
2. 在一个候选银河系势中用 AGAMA 积分，并等时间采样；
3. 每个轨道点继承其种子星的代表性权重 `w`；
4. 将轨道点放入相同的 `(R, z, phi)` 网格，以精确柱坐标体积换算密度；
5. 在全部 phi bin 上只拟合一个共同的模型幅度；
6. 逐 phi 计算密度残差和 chi-square；可选地加入三个速度分量的逐星似然。
...
给出使用完全相同边界生成的观测密度文件。加入 Zhu 的速度项可使用
`--include-velocity`。
